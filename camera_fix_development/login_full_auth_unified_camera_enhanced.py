...
try:
    ...
    st.success("📱 バーコードスキャン機能が利用可能です")
except (ImportError, OSError, Exception) as e:
    pyzbar_available = False
    st.warning(f"📱 バーコードスキャン機能は利用できません: {str(e)[:100]}")
...
def login_signup_page():
    st.header("令和版バーコードバトラー（カメラ機能強化版）",divider="gray")
...
def main_app():
    ...
    if 'user_profile' in st.session_state and st.session_state.user_profile:
        ...
        # pyzbar状態を表示
        with st.sidebar.expander("📱 カメラ機能状態"):
            if pyzbar_available:
                st.success("✅ pyzbar 利用可能")
                st.info("📸 画像アップロード対応")
            else:
                st.warning("❌ pyzbar 利用不可")
                st.info("✏️ 手動入力のみ")
    else:
    ...
    if st.session_state.page == "main":
        st.title("📚 バーコードキャラクター図鑑（カメラ機能強化版）")
    ...
    # --- スキャン画面 ---
        ...
        if pyzbar_available:
            st.title("📷 バーコードスキャン（カメラ対応版）")
            st.success("📱 カメラでのバーコード読取りが利用可能です！")
        else:
            st.title("📷 バーコードスキャン（手入力版）")
            st.info("📝 pyzbarライブラリの問題により、カメラ機能は無効です。バーコード番号を直接入力してください。")
        
        # カメラ機能または画像アップロード（pyzbar が利用可能な場合）
        if pyzbar_available:
            st.subheader("📸 画像からバーコードを読み取り")
            
            # 画像アップロード
            uploaded_file = st.file_uploader(
                "バーコード画像をアップロードしてください", 
                type=['png', 'jpg', 'jpeg'],
                help="スマートフォンで撮影したバーコード画像をアップロードできます"
            )
                    ...
                    with st.spinner("バーコードを解析中..."):
                        ...
                        if decoded_objects:
                            for obj in decoded_objects:
                                ...
                                st.success(f"🎉 バーコードが見つかりました！")
                                st.write(f"**タイプ**: {barcode_type}")
                                st.write(f"**データ**: {barcode_data}")
                            ...
                            st.warning("⚠️ バーコードが検出されませんでした。画像が鮮明で、バーコード全体が写っているか確認してください。")
        ...
        if st.session_state.get('character_generated') and st.session_state.get('generated_character'):
            ...
            st.image(character_info['image'], use_column_width=True)
            st.write(f"**キャラ詳細**")
...
def main():
    st.set_page_config(
        page_title="令和版バーコードバトラー（カメラ機能強化版）",
        page_icon="📱",
        layout="wide"
    )
