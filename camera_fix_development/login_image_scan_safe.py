...
def scan_barcode_from_uploaded_image(uploaded_file):
    """
    アップロードされた画像からバーコードを読み取る（改良版）
    """
    if not PYZBAR_AVAILABLE:
        st.error("❌ pyzbarライブラリが利用できません。手動入力をご利用ください。")
        st.info("💡 Windowsの場合、Visual C++ Redistributableが必要な場合があります。")
        return None
    ...
    try:
        ...
        if barcodes:
            ...
            for barcode in barcodes:
                ...
                detected_codes.append(f"{barcode_type}: {barcode_data}")
                st.success(f"✅ バーコード検出: {barcode_data} (種類: {barcode_type})")
                return barcode_data  # 最初に見つかったバーコードを返す
        else:
            st.warning("⚠️ バーコードが検出されませんでした。")
            st.info("💡 画像の品質を確認し、バーコードがはっきり写っているか確認してください。")
            return None
...
def get_camera_status():
    ...
    status = {
        ...
        "cv2": CV2_AVAILABLE,
        "camera_available": PYZBAR_AVAILABLE and CV2_AVAILABLE
    }
    return status
...
def login_signup_page():
    st.header("令和版バーコードバトラー（画像スキャン機能付き完全統一版）",divider="gray")
    
    # カメラ機能ステータスの表示
    camera_status = get_camera_status()
    if camera_status["pyzbar"]:
        st.success("📷 画像スキャン機能が利用可能です！")
    else:
        st.warning("⚠️ 画像スキャン機能を使用するにはpyzbarが必要です（手動入力は利用可能）")
...
def main_app():
    ...
    if 'user_profile' in st.session_state and st.session_state.user_profile:
        ...
        # 画像スキャン機能の状態表示
        camera_status = get_camera_status()
        scan_status = "✅ 利用可能" if camera_status["pyzbar"] else "❌ 利用不可"
        st.sidebar.info(f"📷 画像スキャン: {scan_status}")
    ...
    if st.session_state.page == "main":
        st.title("📚 バーコードキャラクター図鑑（画像スキャン機能付き完全統一版）")
        ...
        if camera_status["pyzbar"]:
            ...
            st.warning("⚠️ 画像スキャン機能を使用するには pyzbar が必要です。")
    ...
    # --- スキャン画面（画像アップロード機能付き） ---
        ...
        st.title("📷 バーコードスキャン（画像アップロード機能付き）")
        ...
        if PYZBAR_AVAILABLE:
            ...
            scan_method = "✏️ 手動入力"
            st.info("📝 pyzbarが利用できないため、手動入力のみ利用可能です。")
        
        scanned_barcode = None
        
        if scan_method == "🖼️ 画像アップロード" and PYZBAR_AVAILABLE:
            st.subheader("🖼️ 画像アップロードスキャン")
            st.write("バーコードが写った画像をアップロードしてください。")
...
def main():
    st.set_page_config(
        page_title="令和版バーコードバトラー（画像スキャン機能付き完全統合版）",
        page_icon="📱",
        layout="wide"
    )
