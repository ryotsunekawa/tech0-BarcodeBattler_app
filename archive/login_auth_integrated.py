...
def generate_character_image():
    # 1. 商品情報取得（元のコードから）
    product_json = {
        "codeNumber": st.session_state.get('current_barcode', "4901301446596"),
        "codeType": "JAN",
        "itemName": "アタックZERO パーフェクトスティック 76本入り",  # 実際のAPIから取得する場合は置き換え
...
def login_signup_page():
    st.header("令和版バーコードバトラー（Auth統合版）",divider="gray")
    ...
    # --- スキャン画面 ---
        ...
        if img_file:
            ...
            if results:
                ...
                # セッション状態に保存
                st.session_state.current_barcode = digits
            else:
...
def main():
    st.set_page_config(
        page_title="令和版バーコードバトラー（Auth統合版）",
        page_icon="📱",
        layout="wide"
    )
