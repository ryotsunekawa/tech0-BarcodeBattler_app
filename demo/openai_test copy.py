...
- **小物・持ち物**：商品をモチーフにしたデフォルメ武器・防具を装備
- **姿勢**：戦闘ポーズ（カードバトルゲーム風の構え）
- **背景**：背景全体は必ず灰色（#DAD4CF）を基調とし、地域の特徴（自然や建物など）を取り入れた、カードゲーム用イラスト風背景
- **演出**：戦闘力や特殊技を発動しそうなエフェクト（光、オーラ、数字的な力を感じさせる演出）

さらに、テキストプロンプトには「生成する画像の右下（下20px、右20pxの余白を設けること。フォントサイズは100px、フォントはGenkaimincho風）に
地域「{region}」を表示する」といった内容も追加してください。

注意：生成する英語のテキストプロンプトでは **シングルクオート ' を一切使わずないでください**。
