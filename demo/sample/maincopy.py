...
def generate_character_image(product_json):
    # 1. 商品情報確認
    st.write("商品情報", product_json)  # デバッグ用（不要なら消せます）

    # 以下は既存の OpenAI → Stability API 生成処理
    # product_json['itemName'], product_json['makerName'], product_json['itemImageUrl'] などを利用
    # （中身は今書いてある処理を流用でOK）
...
def main_app():
    ...
    # --- スキャン画面 ---
        ...
        # ▼ 生成ボタン：JAN検索 → JSON保存 → キャラ生成 を一気に実行
        ...
        with col2:
            if st.button("✨ 生成する", use_container_width=True):
                ...
                # （任意）確認用に先頭ヒットを表示したいなら以下を有効化
                # st.subheader("APIレスポンス（先頭ヒット）")
                # st.json(product_json)

                # 5) 生成
                sd_prompt, character_name, image = generate_character_image(product_json)
